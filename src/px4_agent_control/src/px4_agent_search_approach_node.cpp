	...
	namespace px4
	{
		...
		public:
			PX4AgentControl(std::string px4_namespace) : Node("px4_agent_control_node")
			{
				...
				std::regex coords_pattern("\\(([+-]?\\d+\\.?\\d*),\\s*([+-]?\\d+\\.?\\d*)\\)");
				std::smatch match;
		...
		private:
			...
			void vln_response_cb(const std_msgs::msg::String::SharedPtr msg)
			{
				...
				while (std::getline(response_stream, line))
				{
					...
					{
						// Log that move command was ignored because no object was found
						RCLCPP_INFO(get_logger(), "Move command ignored - no object detected, continuing to search");
					}
				}
			...
			void introspector_response_cb(const std_msgs::msg::String::SharedPtr msg)
			{
				...
				{
					is_object_found_ = false;
					RCLCPP_INFO(get_logger(), "No object detected - will only rotate until object is found");
				}
			}
		};
		...
		{
			float dx = goal_x - vehicle_lp_.x;
			float dy = goal_y - vehicle_lp_.y;
			float goal_distance = std::sqrt(dx * dx + dy * dy);

			RCLCPP_INFO(get_logger(), "Distance to goal: %.2f", goal_distance);

			if (goal_distance < uosm::px4::GOAL_TOLERANCE)
			{
				RCLCPP_INFO(get_logger(), "Goal reached, mission complete!");
		...
		void PX4AgentControl::publish_vln_query()
		{
			...
			// Compose query - add object detection status
			query_content = mission_objective_ +
							" \nCurrent position: (" + pos + ")." +
							" \nCurrent heading: (" +
							std::to_string(vehicle_lp_.heading * RAD2DEG) + ")." +
							" \nObject detected: " + (is_object_found_ ? "Yes" : "No");
...
int main(int argc, char *argv[])
{
	...
	{
		RCLCPP_WARN(node->get_logger(), "NODE NOT INITIALIZED");
	}
